// src/components/student/users/UserSettings.jsx
  ...
  const handleAddPassword = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setPassword('');
        setConfirmPassword('');
        const updatedUser = { ...user, has_password: true, auth_method: 'both' };
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-[#3e2f7f] h-16 px-8 flex items-center justify-between shadow-md">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={onLogout} className="bg-white text-[#3e2f7f] px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-100">
            Logout
          </button>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto py-12 px-4">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 mb-6">
          ...
          <div className="flex items-center gap-6 mb-6">
            {user.avatar_url ? (
              <img src={user.avatar_url} alt="Profile" className="h-20 w-20 rounded-full" />
            ) : (
            ...
            <div>
              ...
              <div className="mt-2">
                {user.auth_method === 'google' && !user.has_password && (
                  <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">Signed in with Google</span>
                )}
                {user.auth_method === 'manual' && (
                  <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">Email/Password Account</span>
                )}
                {user.auth_method === 'both' && (
                  <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800">Google + Email/Password</span>
                )}
              </div>
            </div>
          </div>
        ...
        {user.auth_method === 'google' && !user.has_password && (
          <div className="bg-white rounded-2xl shadow-lg p-8">
            ...
            <p className="text-gray-600 mb-6">Add a password to enable email/password login in addition to Google sign-in.</p>
            <form onSubmit={handleAddPassword} className="space-y-4">
              <div>
                <label htmlFor="password" className="block text-sm font-medium text-gray-700 mb-2">New Password</label>
                <input id="password" type="password" required
                  className="w-full px-4 py-3 bg-[#e8eaf6] border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#4d4398] text-sm text-gray-900"
        ...
        {user.has_password && (
          <div className="bg-white rounded-2xl shadow-lg p-8">
            ...
            <p className="text-gray-600 mb-4">Your account supports multiple login methods:</p>
            <ul className="space-y-2">
              <li className="flex items-center gap-2"><span className="text-green-600">✓</span><span className="text-gray-700">Email and Password</span></li>
              {user.auth_method === 'both' && (
                <li className="flex items-center gap-2"><span className="text-green-600">✓</span><span className="text-gray-700">Google Sign-In</span></li>
              )}
            </ul>
          </div>
        )}
